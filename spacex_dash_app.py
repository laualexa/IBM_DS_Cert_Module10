...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                value='ALL',
                                placeholder="Select a Launch Ssite here",
                                searchable=True
                                ),
...
def get_success_payload_scatter_chart(site,payload_range):
    sel_min = payload_range[0]
    sel_max = payload_range[1]
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(sel_min,sel_max)]
    if site == 'ALL':
        fig = px.scatter(filtered_df,x="Payload Mass (kg)", y="class", color="Booster Version Category", title=f'Payload Mass(kg) vs Outcome for all sites')
    else:
        filtered_dfb = filtered_df[filtered_df['Launch Site']==site]
        fig = px.scatter(filtered_dfb,x="Payload Mass (kg)", y="class", color="Booster Version Category", title=f'Payload Mass(kg) vs Outcome for {site}')
    return fig
